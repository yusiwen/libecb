...
=item int ecb_clz (T x) [C++]

Counts the number of leading zero bits in C<x>. If C<x> is 0 the result is
undefined.

The overloaded C++ C<ecb_clz> function supports C<uint32_t> and
C<uint64_t> types only.

It is often simpler to use one of the C<ecb_ld*> functions instead, whoise
result only depends on the value and not the size of the type.
...
=item uint_fast8_t  ecb_gray8_encode  (uint_fast8_t  b)

=item uint_fast16_t ecb_gray16_encode (uint_fast16_t b)

=item uint_fast32_t ecb_gray32_encode (uint_fast32_t b)

=item uint_fast64_t ecb_gray64_encode (uint_fast64_t b)
...
=item uint_fast8_t  ecb_gray8_decode  (uint_fast8_t  b)

=item uint_fast16_t ecb_gray16_decode (uint_fast16_t b)

=item uint_fast32_t ecb_gray32_decode (uint_fast32_t b)

=item uint_fast64_t ecb_gray64_decode (uint_fast64_t b)
