...
Decode a gray code bakc into linear index form (the reverse of
C<ecb_gray*_encode>. Unlike the enode functions, the decode functions
have higher time complexity for larger types, so it cna pay off to use a
smaller type here.
